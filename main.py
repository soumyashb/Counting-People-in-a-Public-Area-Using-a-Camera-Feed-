...
cap = cv2.VideoCapture("crowd3jwt.mp4")
unique_ids = set()
...
def save_count(count):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO crowd_counts (count) VALUES (?)",
        (count,)
    )
    conn.commit()
    conn.close()

# Save data once every X seconds
...
cap = cv2.VideoCapture("CrowdTest.mp4")
unique_ids = set()
